				 	...
				 	if (over == false) {
						Platform.runLater(new Runnable(){
							...
							public void run() {
								...
								//YSS EDIT
								//
								//Removed automatic loading of next video to loop single video
								//
								//YSS EDIT
								else {
									...
									//YSS EDIT
							        //
							        //Made it so that player loops the video when in state 0, and player next
									// video when plater state is 1
							        //
							        //YSS EDIT
									else {
										Platform.runLater(new Runnable(){
											@Override
											public void run() {
												int nextIndex = player.getCurrentIndex() + player.getState();
												// If more videos to be loaded then load
												player.loadVideo(nextIndex);
												player.setCurrentIndex(nextIndex);
												player.in_error = false;
